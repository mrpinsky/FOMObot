...
setDMChannel uid cid =
    void $ R.liftRedis $ R.set (dmChannelKey uid) dmChannelValue
  where
    dmChannelValue = encodeUtf8 $ view Slack.getId cid

isDMChannel :: Slack.UserId -> Slack.ChannelId -> Bot Bool
isDMChannel uid cid =
    maybe False (cid ==) <$> getDMChannel uid

getDMChannel :: Slack.UserId -> Bot (Maybe Slack.ChannelId)
getDMChannel uid =
    either (const Nothing) (fmap cidFromByteString) <$> fetchFromRedis uid
  where
